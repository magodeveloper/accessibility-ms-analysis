...
<#
.SYNOPSIS
    Script completo para gestión de tests del microservicio Analysis con dashboard dinámico
.DESCRIPTION
...
.EXAMPLE
    .\manage-tests.ps1 test -Filter "Analysis.Application"
    Ejecuta solo tests que contengan "Analysis.Application"
#>
...
$ProjectName = "Analysis Microservice"
$SolutionFile = "Analysis.sln"
$TestProject = "src\Analysis.Tests\Analysis.Tests.csproj"
...
function Get-CoverageData {
    ...
    try {
        ...
        # Procesar assemblies (solo Analysis, excluyendo Infrastructure)
        foreach ($package in $xml.coverage.packages.package) {
            if ($package.name -like "Analysis.*" -and $package.name -notlike "*Infrastructure*") {
        ...
        <div class="details-grid">
            <div class="detail-card">
                ...
                <div class="assembly-row" style="background: #e8f5e8; padding: 8px; border-radius: 8px;">
                    <div class="assembly-name">✅ Analysis.Infrastructure</div>
                    <div class="assembly-coverage">
...
function Show-Help {
    Write-Banner "📚 AYUDA - GESTOR DE TESTS ANALYSIS MICROSERVICE"
...
PARÁMETROS:
    -Filter <filtro>        Filtra tests específicos (ej: "Analysis.Application")
...
EJEMPLOS:
    ...
    .\manage-tests.ps1 test -Filter "Analysis.Application" 
        Ejecuta solo tests de Analysis.Application
