...
public class ResultService(AnalysisDbContext db) : IResultService
{
    ...
    public async Task<IEnumerable<ResultDto>> GetByLevelAsync(string level)
    {
        return await _db.Results
            .AsNoTracking()
            .Where(r => r.Level.ToString().Equals(level, StringComparison.CurrentCultureIgnoreCase))
    ...
    public async Task<IEnumerable<ResultDto>> GetBySeverityAsync(string severity)
    {
        return await _db.Results
            .AsNoTracking()
            .Where(r => r.Severity.ToString().Equals(severity, StringComparison.CurrentCultureIgnoreCase))
            .Select(r => ToReadDto(r))
            .ToListAsync();
    }
    ...
    public async Task DeleteAllAsync()
    {
        ...
        {
            _ = await _db.Database.ExecuteSqlRawAsync("ALTER TABLE RESULTS AUTO_INCREMENT = 1");
        }
    }

    private static ResultDto ToReadDto(ResultEntity r) => new(
                r.Id,
                r.AnalysisId,
                r.WcagCriterionId,
                r.WcagCriterion,
