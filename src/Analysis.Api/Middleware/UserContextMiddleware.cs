...
public class UserContextMiddleware(RequestDelegate next, ILogger<UserContextMiddleware> logger)
{
    ...
    public async Task InvokeAsync(HttpContext context, IUserContext userContext)
    {
        try
        {
            ...
            if (userContext is not UserContext userContextImpl)
            {
            ...
            else if (context.User?.Identity?.IsAuthenticated == true)
            {
                ...
                var userIdClaim = claims.FirstOrDefault(c => c.Type is "sub" or "nameid" or "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier")?.Value;
                var emailClaim = claims.FirstOrDefault(c => c.Type == "email")?.Value;
                var nameClaim = claims.FirstOrDefault(c => c.Type == "name")?.Value;
                var roleClaim = claims.FirstOrDefault(c => c.Type is "role" or "http://schemas.microsoft.com/ws/2008/06/identity/claims/role")?.Value;
